...
import { getRaftNode } from './raft-service';
...
export const publishMessage = async (
  ...
  // Check if we need to use Raft consensus
  const raftNode = getRaftNode();
  if (raftNode && !process.env.IS_LOCAL) {
    try {
      // If we're using Raft, we should ensure this operation is processed by the leader
      if (!raftNode.isLeader()) {
        // We're not the leader, get the leader endpoint
        const leaderEndpoint = raftNode.getLeaderEndpoint();
        
        if (leaderEndpoint) {
          logger.debug('Forwarding publish request to leader', { 
            leader: leaderEndpoint, 
            topicId 
          });
          
         
          logger.warn('Leader forwarding not implemented, processing locally', {
            topicId,
            leader: leaderEndpoint
          });
        }
        ...
        logger.debug('Submitting publish command to Raft consensus', { topicId });
        
        const success = await clusterService.submitCommand({
          action: 'PUBLISH',
          topicId,
          request
        });
        ...
        if (!success) {
          throw Errors.serviceUnavailable('Failed to achieve consensus for message publishing');
        }
      }
    } catch (error) {
  
      logger.error('Raft consensus error, falling back to direct processing', { error, topicId });
    }
  }


  return processPublishMessage(topicId, request, topic);
};


const processPublishMessage = async (
  topicId: string,
  request: PublishMessageRequest,
  topic: any
): Promise<PublishMessageResponse> => {
